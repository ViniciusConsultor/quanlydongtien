...
namespace Quanlydongtien
{
    class db
    {
        ...
        public db(string Namedb, string dbuser, int dbport)
        {
            string cnnString;
            ...
            cnnString = "Provider=OleMySql.MySqlSource.1;";
            cnnString =  cnnString + "Data Source= \"localhost, " + dbport.ToString();
            cnnString = cnnString + "\";User ID=" + dbuser + ";Initial Catalog=" + Namedb;
            MyConnection.ConnectionString = cnnString;
            try
            {
                MyConnection.Open();
            ...
            try
            {
                if (oleReader.Read())                    
                {
                    ...
                    for (i = 0; i < oleReader.FieldCount; i++)
                    {
                        ...
                        dtGrid.Columns[columnName].AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells;
                        value = oleReader.GetValue(i).ToString();
